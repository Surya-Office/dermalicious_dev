...
final class Klinik extends CI_Controller
{
    ...
    public function list_klinik()
    {
        ...
        foreach ($list as $klinik) {
            ...
            $row[] = "<button class='btn btn-info' onclick='getDetail(this)' data-id='".$klinik['id_klinik']."'><i class='fas fa-eye'></i></button>
                      <button class='btn btn-warning' onclick='getEdit(this)' data-id='".$klinik['id_klinik']."'><i class='fas fa-pencil-alt'></i></button>
                      <button onclick=deleteConfirm('".site_url().'master/klinik/DeleteKlinik/'.$klinik['id_klinik']."') class='btn btn-danger'><i class='fas fa-trash'></i></button>";
    ...
    function insertDataKlinik() {
        ...
        $last_number += 1;
        // var_dump($last_number);die; 
        if ($post['perusahaan'] == 1) {
            $new_id = "01.02.".$last_number;
