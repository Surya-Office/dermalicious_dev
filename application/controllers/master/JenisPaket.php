...
final class JenisPaket extends CI_Controller 
{
    ...
    public function list_jenis_paket()
    {
        ...
        foreach ($list as $jenis) {
            ...
            $row[] = "<button onclick='getEdit(this)' data-id='".$jenis['id_jenis_paket']."' class='btn btn-warning'><i class='fas fa-pencil-alt'></i></button>
                      <button onclick=deleteConfirm('".site_url().'master/JenisPaket/deleteJenisPaket/'.$jenis['id_jenis_paket']."') class='btn btn-danger'><i class='fas fa-trash'></i></button>";
