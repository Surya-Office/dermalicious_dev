...
final class PeriodePaket extends CI_Controller
{
    ...
    public function list_periode_paket()
    {
        ...
        foreach ($list as $periode) {
            ...
            $row[] = "<button class='btn btn-warning' onclick='getEdit(this)' data-id='".$periode['id_periode_paket']."'><i class='fas fa-pencil-alt'></i></button>
                      <button onclick=deleteConfirm('".site_url().'master/PeriodePaket/DeletePeriode/'.$periode['id_periode_paket']."') class='btn btn-danger'><i class='fas fa-trash'></i></button>";
