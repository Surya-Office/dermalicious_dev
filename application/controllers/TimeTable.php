...
class TimeTable extends CI_Controller
{
    ...
    public function list_timetable_summary()
    {
        ...
        foreach ($timetable as $key) {
            ...
            $getslimmingbydate = $this->db->select('sum(jml_lunch) + sum(jml_dinner) as total')->get_where('tbl_transaksi_pengiriman', ['jenis_paket' => 'slimming', 'tgl_pengiriman' => $key['tgl_pengiriman']])->row()->total;
            $gethealthybydate = $this->db->select('sum(jml_lunch) + sum(jml_dinner) as total')->get_where('tbl_transaksi_pengiriman', ['jenis_paket' => 'healthy', 'tgl_pengiriman' => $key['tgl_pengiriman']])->row()->total;
            $gethampersbydate = $this->db->select('sum(jml_lunch) + sum(jml_dinner) as total')->get_where('tbl_transaksi_pengiriman', ['jenis_paket' => 'hampers', 'tgl_pengiriman' => $key['tgl_pengiriman']])->row()->total;
            $getpromotionbydate = $this->db->select('sum(jml_lunch) + sum(jml_dinner) as total')->get_where('tbl_transaksi_pengiriman', ['jenis_paket' => 'promotion', 'tgl_pengiriman' => $key['tgl_pengiriman']])->row()->total;
            $getcorporatebydate = $this->db->select('sum(jml_lunch) + sum(jml_dinner) as total')->get_where('tbl_transaksi_pengiriman', ['jenis_paket' => 'corporate', 'tgl_pengiriman' => $key['tgl_pengiriman']])->row()->total;
